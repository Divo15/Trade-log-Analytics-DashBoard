"""NIFTY monthly-expiry 7-DTE protected short-straddle parameter sweep.

At the first quote at or after the configured entry time on DTE 7, sell the ATM
...
tests 192 target, stop, wing-distance, and entry-time combinations. It assumes
one 65-unit lot, 0.5% adverse slippage on every fill, zero fees, no re-entry, and
uses only current or earlier option quotes. Select the Monthly dataset because
the other expiry buckets do not contain both DTE 7 and DTE 0 for one lifecycle.
"""
...
# 4 × 4 × 4 × 3 = 192 independent combinations. The dashboard executes one
# mapping at a time and ranks the resulting raw executions.
TAKE_PROFIT_PCTS = (0.25, 0.40, 0.55, 0.70)
STOP_LOSS_MULTIPLES = (0.75, 1.00, 1.25, 1.50)
PROTECTIVE_WING_POINTS = (200, 300, 400, 500)
ENTRY_TIMES = ("09:20", "09:35", "09:50")
SWEEP_PARAMETER_SETS = tuple(
    {
        "take_profit_pct": take_profit,
        "stop_loss_multiple": stop_loss,
        "fixed_wing_points": wing_points,
        "entry_time": entry_time,
    }
    for take_profit, stop_loss, wing_points, entry_time in product(
        TAKE_PROFIT_PCTS,
        STOP_LOSS_MULTIPLES,
        PROTECTIVE_WING_POINTS,
        ENTRY_TIMES,
    )
)
...
STRATEGY_NAME = "NIFTY 7-DTE protected short straddle sweep"
...
def run_strategy(context: StrategyContext) -> dict[str, Any]:
    ...
    if 7 not in available_dtes or 0 not in available_dtes:
        raise ValueError(
            "This 7-DTE strategy requires a dataset containing both DTE 7 entries "
            "and DTE 0 exits. Select the Monthly dataset."
        )
    ...
    return {
        ...
        "metadata": {
            ...
            "engine": "embedded 7-DTE protected short-straddle engine",
            "required_dataset": "Monthly (must contain DTE 7 through DTE 0)",
