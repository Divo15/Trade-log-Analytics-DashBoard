"""Dashboard sweep: finalized E1+R1 four-slot NIFTY straddle.
...
RUN_MODE = "sweep"
ENTRY_STRADDLE_PREMIUM_PCTS = (10.0, 15.0, 20.0, 25.0, 30.0)
SWEEP_PARAMETER_SETS = tuple(
    {"entry_straddle_premium_pct": value}
    for value in ENTRY_STRADDLE_PREMIUM_PCTS
)
ALLOWED_PARAMETERS = {"entry_straddle_premium_pct"}
...
class _DayProgress:
    """Report one update per trading day without touching strategy state."""
    ...
    def __init__(self, total: int, reporter=None):
        ...
        if reporter is not None:
            ...
            try:
                ...
                self.bar = tqdm(
                    total=total, desc="NIFTY Backtest", unit=" trading day",
                    dynamic_ncols=True, mininterval=1.0, smoothing=0.1,
                    bar_format=(
...
def _parse_datetimes(values: pd.Series) -> pd.Series:
    """Batch the dataset's standard format; preserve scalar parsing otherwise."""
...
def _parquet_sources(path: Path) -> list[Path]:
    """List cache dependencies without reading market data at import time."""
    return [path] if path.is_file() else sorted(path.rglob("*.parquet"))


def _load_market_data(selected: Path, loader=None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    read_summary = lambda: pd.read_parquet(summary_path, columns=summary_columns)
    read_chain = lambda: pd.read_parquet(chain_path, columns=chain_columns)
    if loader is None:
        summary = read_summary()
        chain = read_chain()
    else:
        summary = loader.read_frame(
            _parquet_sources(summary_path),
            ("e1r1-summary-columns-v1", tuple(summary_columns)),
            read_summary,
        )
        chain = loader.read_frame(
            _parquet_sources(chain_path),
            ("e1r1-chain-columns-v1", tuple(chain_columns)),
            read_chain,
        )
...
def _build_days(
    ...
    chain: pd.DataFrame,
    selected_premium_pct: float = SELECTED_PREMIUM_PCT,
...
def run_strategy(context):
    """Run one deterministic four-slot premium-selection variation."""
    ...
    if set(parameters) != ALLOWED_PARAMETERS:
        raise ValueError(
            "The dashboard must supply exactly entry_straddle_premium_pct for this sweep"
        )
    selected_pct = float(parameters["entry_straddle_premium_pct"])
    if selected_pct <= 0:
        raise ValueError("entry_straddle_premium_pct must be positive")

    summary, chain = _load_market_data(
        Path(context.market_data), getattr(context, "market_data_loader", None)
    )
    days = _build_days(summary, chain, selected_premium_pct=selected_pct)
    completed: list[dict[str, Any]] = []
    snapshots: list[dict[str, Any]] = []
    ...
    try:
        for day in days:
            day_trades, day_snapshots = _run_day(
                day,
                str(context.run_id),
    ...
    return {
        ...
        "metadata": {
            "strategy_name": "E1-R1 four-slot combined-premium straddle sweep",
            ...
            "strike_rule": f"closest valid CE/PE premium to {selected_pct:g}% of ATM straddle premium",
            ...
            "active_slots": MAX_ACTIVE_SLOTS,
            "margin_basis_rs": LOT_MARGIN_RS,
