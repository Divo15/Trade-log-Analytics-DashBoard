"""NIFTY weekly-expiry 6-DTE hedged short-straddle parameter sweep.

At the first quote at or after the configured entry time on DTE 6, sell the ATM
call and put and buy equidistant protective wings. Exit the complete four-leg
structure at its profit target, structure stop, or on DTE 0 at 15:15. The sweep
tests 60 combinations of target, stop-loss, protective-wing distance, and zero
through two re-entries after a target or stop exit. It assumes one 65-unit lot,
0.5% adverse slippage on every fill, zero fees, and uses only current or earlier
option quotes. Select the Weekly dataset because it contains the complete DTE 6
through DTE 0 lifecycle for the weekly expiry.
"""
...
# 5 take-profit levels × 2 stop-loss levels × 2 wing distances × 3 re-entry
# limits = 60 independent backtests. Every re-entry remains eligible only after
# a full structure take-profit or stop-loss exit.
TAKE_PROFIT_PCTS = (0.20, 0.30, 0.40, 0.50, 0.60)
STOP_LOSS_MULTIPLES = (0.75, 1.25)
WING_DISTANCES = (200, 400)
MAX_REENTRY_OPTIONS = (0, 1, 2)
SWEEP_PARAMETER_SETS = tuple(
    {
        "take_profit_pct": take_profit,
        "stop_loss_multiple": stop_loss,
        "fixed_wing_points": wing_distance,
        "max_reentries": max_reentries,
    }
    for take_profit, stop_loss, wing_distance, max_reentries in product(
        TAKE_PROFIT_PCTS,
        STOP_LOSS_MULTIPLES,
        WING_DISTANCES,
        MAX_REENTRY_OPTIONS,
...
def load_summary(base: Path, start_date: str, end_date: str, loader=None) -> pd.DataFrame:
    read = lambda: _read_summary(base, start_date, end_date)
    if loader is None:
        return read()
    return loader.read_frame(
        [base / "nifty_summary.parquet"],
        ("weekly-summary-v1", start_date, end_date), read,
    )


def _read_summary(base: Path, start_date: str, end_date: str) -> pd.DataFrame:
    con = duckdb.connect()
...
def load_cycle_chain(con, chain_glob, start_ts, end_ts, loader=None):
    read = lambda: _read_cycle_chain(con, chain_glob, start_ts, end_ts)
    if loader is None:
        return read()
    return loader.read_frame(
        sorted(Path(chain_glob).parent.glob("*.parquet")),
        ("weekly-chain-v1", start_ts, end_ts), read,
    )


def _read_cycle_chain(con, chain_glob, start_ts, end_ts):
...
def option_series(chain, strike, side, loader=None):
    prepare = lambda: _prepare_option_series(chain, strike, side)
    if loader is None:
        return prepare()
    return loader.prepare_frame(("weekly-option-v1", strike, side), [chain], prepare)


def _prepare_option_series(chain: pd.DataFrame, strike: int, side: str) -> pd.DataFrame:
...
def build_joined_path(chain, summary_slice, short_call, short_put, long_call, long_put, loader=None):
    prepare = lambda: _build_joined_path(chain, summary_slice, short_call, short_put, long_call, long_put, loader)
    if loader is None:
        return prepare()
    return loader.prepare_frame(
        ("weekly-path-v1", short_call, short_put, long_call, long_put),
        [chain, summary_slice], prepare,
    )


def _build_joined_path(chain, summary_slice, short_call, short_put, long_call, long_put, loader=None):
    ...
    for strike, side in (
        ...
        series = option_series(chain, strike, side, loader)
        if series.empty:
            return pd.DataFrame()
...
def evaluate_event(
    ...
    # Preserve the scalar evaluator's arithmetic order and float64 precision.
    call_prices = path[f"CE_{short_call}"].to_numpy(dtype=float)
    put_prices = path[f"PE_{short_put}"].to_numpy(dtype=float)
    exit_debit = (
        call_prices * (1 + SLIPPAGE)
        + put_prices * (1 + SLIPPAGE)
        - path[f"CE_{long_call}"].to_numpy(dtype=float) * (1 - SLIPPAGE)
        - path[f"PE_{long_put}"].to_numpy(dtype=float) * (1 - SLIPPAGE)
    )
    open_pnl = entry_credit_points - exit_debit
    ...
    def close_at(row: pd.Series, reason: str) -> tuple[pd.Series, float, str]:
        ...
        for mark_ts, mark_pnl in zip(observed["ts"], observed["open_pnl_points"]):
            timestamp = pd.Timestamp(mark_ts)
            snapshot_sink[timestamp] = {
                "timestamp": timestamp,
                "realized_pnl": realized_before_points * lot_size,
                "unrealized_pnl": float(mark_pnl) * lot_size,
            }
    ...
    # Compare all observed bars, then choose the earliest event. Ordering of
    # masks preserves the original priority when several events share a bar.
    candidates = []
    def first_hit(mask, reason):
        hits = np.flatnonzero(np.asarray(mask)[1:])
        if hits.size:
            candidates.append((int(hits[0]) + 1, len(candidates), reason))

    timestamps = pd.to_datetime(path["ts"])
    if max_hold_minutes is not None:
        first_hit(timestamps >= entry_ts + timedelta(minutes=max_hold_minutes), "max_hold_exit")
    if force_exit_dte is not None:
        dtes = path["DTE"].to_numpy(dtype=int)
        force = dtes < force_exit_dte
        same_dte = dtes == force_exit_dte
        if square_off_time is not None:
            same_dte = same_dte & (timestamps.dt.time >= square_off_time).to_numpy()
        first_hit(force | same_dte, "time_exit")
    first_hit(open_pnl >= tp_threshold, "take_profit")
    if profit_lock_trigger_pct is not None:
        triggered = open_pnl >= entry_credit_points * profit_lock_trigger_pct
        triggered[0] = False  # Entry bar never arms the lock.
        armed = np.logical_or.accumulate(triggered)
        first_hit(armed & (open_pnl <= entry_credit_points * profit_lock_exit_pct), "profit_lock")
    if use_leg_stop:
        first_hit((call_prices * (1 + SLIPPAGE) >= call_leg_sl)
                  | (put_prices * (1 + SLIPPAGE) >= put_leg_sl), "leg_stop")
    else:
        first_hit(path["exit_debit_points"].to_numpy() >= sl_debit, "stop_loss")
    if roll_trigger_pct is not None:
        center_strike = (short_call + short_put) / 2.0
        first_hit((path["DTE"].to_numpy(dtype=int) > no_roll_last_dte)
                  & (np.abs(path["future_close"].to_numpy(dtype=float) - center_strike)
                     >= wing_distance * roll_trigger_pct), "roll")
    if candidates:
        index, _, reason = min(candidates)
        return close_at(path.iloc[index], reason)
...
def select_cycle_entry(
    ...
    if args.entry_exact_dte is not None:
        exact_rows = cycle_summary[
            cycle_summary["DTE"].astype(int) == int(args.entry_exact_dte)
        ]
        entry_dates = list(dict.fromkeys(exact_rows["trade_date"].tolist()))
        if not entry_dates:
    ...
    for entry_date in entry_dates:
        ...
        if args.entry_exact_dte is not None:
            candidate_summary = candidate_summary[
                candidate_summary["DTE"].astype(int) <= int(args.entry_exact_dte)
            ].copy()
...
def run_backtest(args: SimpleNamespace) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, list[dict[str, Any]]]:
    base = Path(args.base)
    loader = getattr(args, "market_data_loader", None)
    summary = add_cycle_id(load_summary(base, args.start_date, args.end_date, loader), args.cycle_reset_dte_jump)
    con = duckdb.connect()
    con.execute("PRAGMA disable_progress_bar")
    ...
    segment_rows = []
    leg_rows = []
    equity_snapshots: dict[pd.Timestamp, dict[str, Any]] = {}
    portfolio_realized_points = 0.0
    ...
    for cycle_id, cycle in summary.groupby("cycle_id", sort=True):
        ...
        cycle_chain = load_cycle_chain(con, chain_glob, entry_row["ts"], expiry_row["ts"], loader)
        ...
        while not remaining_summary.empty:
            ...
            portfolio_realized_points += pnl_points
    ...
    ordered_snapshots = [equity_snapshots[key] for key in sorted(equity_snapshots)]
    if trades_df.empty:
        return trades_df, trades_df, segments_df, legs_df, ordered_snapshots
    ...
    return trades_df, monthly, segments_df, legs_df, ordered_snapshots
...
STRATEGY_NAME = "NIFTY weekly 6-DTE hedged short straddle — hard-exit sweep"
...
def run_strategy(context: StrategyContext) -> dict[str, Any]:
    ...
    with duckdb.connect() as coverage:
        available_dtes = {
            int(row[0])
            for row in coverage.execute(
                "SELECT DISTINCT CAST(DTE AS BIGINT) FROM read_parquet(?)",
                [str(summary_file)],
            ).fetchall()
        }
    if 6 not in available_dtes or 0 not in available_dtes or max(available_dtes) > 6:
        raise ValueError(
            "This weekly 6-DTE strategy requires the complete DTE 6 through DTE 0 "
            "weekly-expiry lifecycle. Select the Weekly dataset."
        )
    ...
    args.market_data_loader = getattr(context, "market_data_loader", None)
    ...
    _, _, _, completed_legs, equity_snapshots = run_backtest(args)
    ...
    observed_equity = [
        {
            "timestamp": timestamp_with_timezone(item["timestamp"], timezone),
            "realized_pnl": item["realized_pnl"],
            "unrealized_pnl": item["unrealized_pnl"],
        }
        for item in equity_snapshots
    ]
    ...
    return {
        ...
        "equity_snapshots": observed_equity or None,
        "metadata": {
            "strategy_name": STRATEGY_NAME,
            "engine": "embedded weekly 6-DTE hedged short-straddle engine",
            "required_dataset": "Weekly (must contain DTE 6 through DTE 0)",
