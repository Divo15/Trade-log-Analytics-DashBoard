...
function renderYearlyPerformance() {
  ...
  $("totalRoi").textContent = valid
    ? `Total ROI: ${number.format(state.data.overview.net_pnl / capital * 100)}% · Net profit ${money.format(state.data.overview.net_pnl)} ÷ capital ${money.format(capital)}`
  ...
  $("yearlyRows").innerHTML = yearly.map((row) => {
    ...
    return `<tr><td>${escapeHtml(row.year)}</td><td class="numeric ${pnlClass}">${money.format(row.net_pnl)}</td><td class="numeric ${bestClass}">${money.format(row.best_day)}</td><td class="numeric ${drawdownClass}">${money.format(row.max_drawdown)}</td><td class="numeric ${roiClass}">${row.roi == null ? "—" : `${number.format(row.roi)}%`}</td><td class="numeric">${row.losing_days}</td><td class="numeric">${row.traded_days}</td></tr>`;
  }).join("");
}
